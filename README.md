# Zotero DataCheck

Zotero DataCheck 是一个运行在 Zotero 内部的表格数值筛查插件。它的目标不是替用户下结论，而是把论文表格里值得进一步人工复核的异常模式先标出来，帮助你更快完成初步检查。

## 适用场景

- 在 Zotero 阅读论文时，快速筛查表格中的可疑数值模式
- 为审稿、复核、文献阅读和方法学习提供一个低门槛的辅助工具
- 在不离开 Zotero 的前提下，对选中的表格文本生成一份可解释的检查报告

## 你可以用它做什么

- 对 PDF 阅读器中选中的表格文本做本地分析
- 自动重建表格的行列结构，并标准化数字、百分比和 p 值表达式
- 标记重复行、重复数值序列、重复数值列、异常百分比、异常 p 值和高重复值列
- 在样本条件满足时给出本福特偏离提示
- 输出带摘要、检测项、发现明细和重建诊断的报告

## 安装

1. 打开 GitHub Releases 页面并下载最新的 xpi 安装包。
2. 在 Zotero 中打开 工具 -> 插件。
3. 点击右上角齿轮按钮，选择 Install Add-on From File...。
4. 选择下载好的 xpi 文件并完成安装。

当前发布面向 Zotero 7 及更新版本。

## 使用方法

1. 在 Zotero 中打开论文 PDF。
2. 在 PDF 阅读器里选中需要检查的表格文本。
3. 在选区弹窗中点击 DataCheck 分析命令。
4. 等待插件完成表格重建和检测。
5. 在报告窗口中查看摘要、命中项和重建诊断，再回到原文做人工复核。

## 当前支持的检测项

- 重复行
- 重复数值序列
- 重复数值列
- 高重复值列
- 越界百分比
- 越界 p 值
- 本福特偏离提示（仅在样本条件满足时启用）

## 结果如何解读

- DataCheck 输出的是风险信号，不是对论文或作者的结论性判断。
- 报告中的命中项只表示“值得继续检查”，不等于数据一定存在问题。
- 如果报告里同时出现重建诊断，优先先确认选区和表格重建是否准确。
- 对样本量要求较高的检测项，插件会在不适用时明确提示原因。

## 数据与隐私

- 分析在本地 Zotero 环境中完成。
- 正式版本不依赖外部 Python 服务。
- 不需要把表格数据上传到额外的云端服务才能运行检测。

## 适用范围与限制

- 当前更适合带有文本层、版式较规整的论文 PDF。
- 当前流程依赖用户先在阅读器中选中文本，不会自动从整篇 PDF 中定位表格。
- 当前不做 OCR；如果 PDF 是扫描件或图片型表格，提取结果可能失败或不完整。
- 选区质量会直接影响表格重建和后续检测结果。

## 当前状态

- 当前版本已经支持从 PDF 阅读器选区直接打开分析报告。
- 现阶段重点是把手动选区分析路径做稳定，再逐步扩展检测器和批处理入口。

## 反馈与文档

- 如果你遇到误报、漏报、无法重建表格或界面问题，欢迎提交 issue。
- 面向开发与实现的规划文档见 [docs/implementation-plan.md](docs/implementation-plan.md)。
