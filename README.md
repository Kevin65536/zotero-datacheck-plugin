...
Zotero DataCheck 是一个运行在 Zotero 内部的表格数值筛查插件。它的目标不是替用户下结论，而是把论文表格里值得进一步人工复核的异常模式先标出来，帮助你更快完成初步检查。
...
- 发布渠道：安装包通过 GitHub Releases 提供下载

## 适用场景

- 在 Zotero 阅读论文时，快速筛查表格中的可疑数值模式
- 为审稿、复核、文献阅读和方法学习提供一个低门槛的辅助工具
- 在不离开 Zotero 的前提下，对选中的表格文本生成一份可解释的检查报告

## 你可以用它做什么

- 对 PDF 阅读器中选中的表格文本做本地分析
- 自动重建表格的行列结构，并标准化数字、百分比和 p 值表达式
...
- 分析在本地 Zotero 环境中完成。
- 正式版本不依赖外部 Python 服务。
- 不需要把表格数据上传到额外的云端服务才能运行检测。

## 适用范围与限制

- 当前更适合带有文本层、版式较规整的论文 PDF。
- 当前流程依赖用户先在阅读器中选中文本，不会自动从整篇 PDF 中定位表格。
- 当前不做 OCR；如果 PDF 是扫描件或图片型表格，提取结果可能失败或不完整。
- 选区质量会直接影响表格重建和后续检测结果。
